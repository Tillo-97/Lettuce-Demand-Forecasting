...
write.csv(lettuce_quantity_per_recipe, file ="lettuce_quantity_per_recipe.csv")

```
We created a new data frame lettuce_quantity_per_recipe which contains the various RecipeId that use lettuce and the quantity of lettuce required by each Recipe.
...
We will look into the estimation error of both models
```{r}
...
We will carry out stationary tests to determine if our time series is stationary.
```{r}
# stationary test
adf.test(training_set_12631)
pp.test(training_set_12631)
kpss.test(training_set_12631)
...
Therefore, having taken all these tests into account, we are confident that we do not need any order differencing and our time series is stationary.

Next, we will need to make choices for p, q, P and Q
```{r}
# acf plot
ggAcf(training_set_12631) # q = 0

# pacf plot
ggPacf(training_set_12631) # p = 0
```
When we plot the ACF, we notice that no lag is outside the 95th percentile. So our q value is likely 0. 
...
We have also performed a Ljung-Box test on the models which is a statistical test for residual autocorrelation. For this test, the null hypothesis is that there is no residual autocorrelation and we reject it if we get a p value smaller than 0.05. For all our models, we see in our results that they have p values above 0.05 so we cannot reject the null hypothesis for any of them.
...
Finally, we will show the results for the best model to forecast the demand of lettuce for Restaurant 12631
```{r}
# Best model for restaurant 12631
restaurant_12631.model.ets.f
autoplot(restaurant_12631.model.ets.f)
```
...
```
From the plot, we can see that the trend can be ignored but the seasonal and error component seem to have an additive parameter
...
plot(restaurant_20974.arima.f2)
lines(fitted(restaurant_20974.arima.f2), col = "blue", lty = 2)
```
...
Finally, we will show the results for the best model to forecast the demand of lettuce for Restaurant 20974
```{r}
# Best model for restaurant 20974
restaurant_20974.model.ets.f
autoplot(restaurant_20974.model.ets.f)
```
...
```
We see that the second ets model has a smaller mean squared and adjusted mean squared error. So, the "MNA" model has a better fit and accuracy as it has a smaller error and AICc. Therefore, we will use this as our ets model. 
...
```{r}
# Take first order difference to make time series stationary
restaurant_46673_ts_diff1 <- diff(training_set_46673, differences = 1)

autoplot(restaurant_46673_ts_diff1)
```
After taking the first order difference, it appears our time series is free of trend or seasonality. To be sure we will carry out stationary tests.
...
Additionally, we have the ndiffs test which tells us how many order differences we need to take to make the time series trend stationary and we can see that the result is 0 meaning we do not  need to take any order differences for the trend. Additionally, for the seasonality stationarity test, we use the nsdiffs test and we see that the result is 1 meaning we only  need to take any 1 order difference for the seasonality. 

Therefore, having taken all these tests into account, we are confident that the time series is now stationary after taking 1 order difference.
...
# pacf plot
ggPacf(restaurant_46673_ts_diff1) # p <= 3
```
...
Finally, we will show the results for the best model to forecast the demand of lettuce for Restaurant 46673
```{r}
# Best model for restaurant 46673
restaurant_46673.model.arima.f
autoplot(restaurant_46673.model.arima.f)
```
